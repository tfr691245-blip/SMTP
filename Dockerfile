...
RUN apk add --no-cache \
    ...
    php82-session php82-curl php82-imap \
...
# 3. APP LOGIC (REAL USAGE TRACKING)
...
function get_real_usage($u, $p) {
    // Connect to Gmail Sent folder directly
    $mbox = @imap_open("{imap.gmail.com:993/imap/ssl}[Gmail]/Sent Mail", $u, $p);
    if (!$mbox) return 0;
    
    // Count emails sent since today (Server time)
    $since = date("d-M-Y"); 
...
function get_auth($u, $p) {
    ...
    fwrite($sock, base64_encode($u)."\r\n"); fgets($sock, 512);
    fwrite($sock, base64_encode($p)."\r\n");
    $res = fgets($sock, 512);
    fwrite($sock, "QUIT\r\n"); fclose($sock);
...
if(isset($_GET['status'])) {
    ...
    echo json_encode([
        'status' => get_auth($user, $pass),
        'used'   => get_real_usage($user, $pass),
        'limit'  => $limit
    ]); exit;
}
...
<head>
    ...
    <style>
        ...
        #preview { background: #050505; border-radius: 12px; padding: 10px; margin-bottom: 12px; min-height: 40px; font-size: 11px; border: 1px dashed #222; color: #555; overflow: hidden; }
    </style>
</head>
<body>
    <div class="card">
        <div class="header">
            <h3 style="margin:0;letter-spacing:1px;font-weight:900;">MASTERSYNC</h3>
    ...
    <script>
        ...
        f.onsubmit = async (e) => {
            ...
            const d = await res.json();
            if(d.status === 'success') { f.reset(); p.innerHTML = 'PREVIEW: (Empty)'; }
